...
function ShowStats()
{
   ...
   if (pass.length == 0)
   {
      Set_Text("Please type your password,<br /> the analysis will start automatically.");
			hideBar();
      window.setTimeout('ShowStats();', 200);
      return;
   } else {
   ...
   if (pass.length > 1)
   {
	 ...
	 aidx = bidx;
      ...
      if (bits < 15)
      {
         r += "<b>Strength:  <font color=red>Very Weak "+bits+"</font></b><br>\n";
		 setProgressBarValue(bits/0.4,"yellow");
      }
      else if (bits < 20)
      {
         r += "<b>Strength:</b>  <font color=red>Weak "+bits+"</font><br>\n";
		 setProgressBarValue(bits/0.4,"red");
      }
      else if (bits < 30)
      {
         r += "<b>Strength:</b>  <font color=brown>Reasonable "+bits+"</font><br>\n";
		 setProgressBarValue(bits/0.4,"brown");
      }
      else if (bits < 40)
      {
         r += "<b>Strength:</b>  <font color=green>Strong "+bits+"</font><br>\n";
		 setProgressBarValue(bits/0.4,"green");
      }
      else
      {
         r += "<b>Strength:</b>  <font color=blue>Very Strong "+bits+"</font><br>\n";
